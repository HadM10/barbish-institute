...
exports.markSessionAsWatched = async (req, res) => {
  try {
    ...
    console.log('Updating session watch status:', { userId, sessionId, isWatched });
...
exports.getSessionProgress = async (req, res) => {
  try {
    ...
    console.log('Found sessions for course:', { courseId, count: sessions.length });
    ...
    const progress = {
      ...
      sessions: sessions.map(session => {
        const userSession = userSessions.find(us => us.sessionId === session.id);
        return {
